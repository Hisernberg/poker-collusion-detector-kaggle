"""
Poker Collusion Detector v7 — Single-Pass Streaming (Ultra-Low Memory)
=======================================================================
Process seats.parquet row-group by row-group WITHOUT keeping hand_seat_data
in memory. For each row group, we have all seats for that row group's hands.
We process per-hand within the row group and update per-pair accumulators.

To handle hands that may span row groups (rare but possible), we keep a small
buffer dict for "in-progress" hands. After each row group, we process hands
whose accumulated player count matches players_dealt from hands.parquet.
...
# Build dict: hand_id -> (phase, big_blind, final_pot, players_dealt)
hands_dict = {}
...
# =========================================================================
# Streaming Pass: Process seats.parquet row-group by row-group
# =========================================================================
print()
print("="*80)
print("Streaming through seats.parquet row-groups")
print("="*80)
...
# Buffer for hands that may span row groups: hand_id -> dict[player_id -> seat_data]
# Will keep this small by processing complete hands ASAP
hand_buffer = {}

def process_hand(hand_id, players_dict):
    """Process a complete hand: enumerate pairs, update accumulators."""
    ...
    for i in range(len(players_list)):
        ...
        for j in range(i+1, len(players_list)):
            ...
            n_pairs_found += 1
    return n_pairs_found

# Stream seats.parquet row groups
...
for rg_idx in range(pf_seats.num_row_groups):
    ...
    for hand_id, group in df.groupby("hand_id"):
        ...
        # Check if this hand is complete (player count matches players_dealt)
        if hand_id in hands_dict:
            expected_n = hands_dict[hand_id][3]  # players_dealt
            if len(players_dict) == expected_n:
                # Complete hand - process it
                pairs = process_hand(hand_id, players_dict)
                total_pairs += pairs
                hands_processed += 1
            else:
                # Hand might span row groups — add to buffer
                if hand_id in hand_buffer:
                    hand_buffer[hand_id].update(players_dict)
                    if len(hand_buffer[hand_id]) == expected_n:
                        pairs = process_hand(hand_id, hand_buffer[hand_id])
                        total_pairs += pairs
                        hands_processed += 1
            ...
            # Hand not in hands_dict (shouldn't happen)
            pass

    if (rg_idx + 1) % 20 == 0 or rg_idx == pf_seats.num_row_groups - 1:
        elapsed = time.time() - t_stream
        print(f"  row group {rg_idx+1}/{pf_seats.num_row_groups}: hands={hands_processed:,}, pairs={total_pairs:,}, buffer={len(hand_buffer):,}, elapsed={elapsed:.1f}s", flush=True)

# Process any remaining buffered hands (assume complete even if not all players)
...
print(f"Streaming done in {time.time()-t_stream:.1f}s. {hands_processed:,} hands, {total_pairs:,} pair-hand records.")
print(f"Buffer remaining: {len(hand_buffer):,}")

# Free buffer and hands_dict
