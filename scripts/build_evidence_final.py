"""Final evidence submission builder - uses heapq directly during streaming."""
...
print("="*80)
print("Building evidence submissions (heapq during streaming)")
print("="*80)
t0 = time.time()
...
print(f"pair_evidence (defaultdict): {len(pair_evidence):,}")
...
for rg_idx in range(pf_seats.num_row_groups):
    ...
    if (rg_idx + 1) % 20 == 0 or rg_idx == pf_seats.num_row_groups - 1:
        ...
        print(f"  rg {rg_idx+1}/{pf_seats.num_row_groups}: hands={hands_scored:,}, pairs_w_ev={len(pair_evidence):,}, elapsed={elapsed:.1f}s", flush=True)
