"""
Poker Collusion Detector v6 — Streaming Aggregation (Low Memory)
================================================================
Process seats row-group by row-group, then iterate per-hand, updating
per-pair running aggregates directly (no intermediate storage).
Keep a top-5 min-heap per pair for evidence selection.

Memory: ~720MB for hand_seat_data + ~100MB for accumulators = under 1GB.
"""
...
# =========================================================================
# PASS 1 — Stream seats.parquet, build hand_seat_data
# =========================================================================
print()
print("="*80)
print("PASS 1 — Streaming seats.parquet to build hand_seat_data")
print("="*80)

needed_players_set = needed_players
...
# Build hand_id -> dict[player_id -> (stack, contrib, net, folded, showdown, won_share)]
...
for rg_idx in range(pf_seats.num_row_groups):
    ...
    mask = df.player_id.isin(needed_players_set)
    df = df[mask]
    rows_read += len(df)
    # Accumulate per-hand
    ...
    if (rg_idx + 1) % 50 == 0 or rg_idx == pf_seats.num_row_groups - 1:
...
print(f"Memory (RSS estimate): ~{len(hand_seat_data) * 0.4 / 1e6:.0f}MB")

# =========================================================================
# Load hands.parquet (full — only 2M rows, fits easily)
# =========================================================================
print()
print("Loading hands.parquet (polars, then convert to dict)...")
import polars as pl
...
# Build dict: hand_id -> (phase, big_blind, final_pot, players_at_showdown)
# Use polars iter_rows for efficient iteration
hands_dict = {}
for row in hands_pl.iter_rows(named=True):
    hands_dict[row["hand_id"]] = (
        row["phase"], int(row["big_blind"]), int(row["final_pot"]), int(row["players_at_showdown"])
    )
del hands_pl
gc.collect()
...
# =========================================================================
# PASS 2 — Iterate per-hand, update per-pair accumulators + top-5 evidence
# =========================================================================
print()
print("="*80)
print("PASS 2 — Per-hand processing with streaming aggregation")
print("="*80)

# Per-pair accumulators (use defaultdict with new dict template)
def new_accumulator():
    return {
        "n_hands": 0,
...
# Per-pair top-5 min-heap: list of (suspicion_score, hand_id)
...
for hand_id, players_dict in hand_seat_data.items():
    ...
    for i in range(n):
        ...
        for j in range(i+1, n):
            ...
            # Asymmetric: one wins, other loses, big diff
            ...
            acc["net1_max"] = max(acc["net1_max"], net1_bb)
            acc["net1_min"] = min(acc["net1_min"], net1_bb)
            acc["net2_sum"] += net2_bb
            acc["net2_sq_sum"] += net2_bb * net2_bb
            acc["net2_max"] = max(acc["net2_max"], net2_bb)
            acc["net2_min"] = min(acc["net2_min"], net2_bb)
            acc["contrib1_sum"] += contrib1_bb
            acc["contrib2_sum"] += contrib2_bb
            acc["stack1_sum"] += stack1_bb
            acc["stack2_sum"] += stack2_bb
            acc["pot_sum"] += pot_bb
            acc["pot_max"] = max(acc["pot_max"], pot_bb)
            acc["abs_net_diff_sum"] += abs_net_diff
            acc["abs_net_diff_max"] = max(acc["abs_net_diff_max"], abs_net_diff)
            ...
            acc["transfer_signal_min"] = min(acc["transfer_signal_min"], transfer)
            acc["asymmetric_outcome_count"] += asymmetric
            acc["passive_outcome_count"] += passive

            # Update top-5 evidence heap (suspicion score)
            suspicion_score = (
                asymmetric * 1.5 +
                passive * 2.0 +
                abs_net_diff * 0.5 +
    ...
    hands_processed += 1
    if hands_processed % 500_000 == 0:
        elapsed = time.time() - t_pass2
        print(f"  processed {hands_processed:,} hands ({pairs_found:,} pair-hand records), elapsed={elapsed:.1f}s", flush=True)

print(f"Pass 2 done in {time.time()-t_pass2:.1f}s. {pairs_found:,} pair-hand records.")

# Free hand_seat_data, hands_dict
del hand_seat_data, hands_dict
gc.collect()
...
# Sort each heap by score descending and emit
